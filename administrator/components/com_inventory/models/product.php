 ...
 * @since  1.6
 */
class InventoryModelProduct extends JModelAdmin
{
	/**
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_inventory.product';
	...
	protected function loadFormData()
	{
		...
		if (empty($data))
		{
			...
			// Support for 'multiple' field
			$data->skirt = json_decode($data->skirt);
		}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__inventory_products');
				$max             = $db->loadResult();
