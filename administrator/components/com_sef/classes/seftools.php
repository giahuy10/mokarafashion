...
class SEFTools
{
    ...
    static function getExtVersion($extension)
    {
     ...
     * @return string
     */
    static function getExtName($extension)
    {
        $xml = SEFTools::getExtXML($extension);
        $name = null;
     ...
     */
    static function getExtXML($extension)
    {
        static $xmls;
    ...
    static function &getExtAcceptVars($option, $includeGlobal = true)
    {
        static $acceptVars = array();
    ...
    static function &getExtFilters($option, $includeGlobal = true)
    {
        ...
        if( !isset($filters[$option]) ) {
            ...
            $query->select('custom_data')->from('#__extensions')->where('type='.$db->quote('sef_ext'))->where('enabled=1')->where('element='.$db->quote($element));
            $db->setQuery($query);
            $row = $db->loadResult();
    ...
    static function &getExtFiltersByVars($option, $includeGlobal = true)
    {
        static $byVars = array();
     ...
     */
    static function &getExtParamsForm($option)
    {
        static $forms;
    ...
    static function &getExtParams($option)
    {
        $db = JFactory::getDBO();
        $element = str_replace('com_', 'ext_joomsef4_', $option);
     ...
     */
    static function getExtsDefaultParamsXML()
    {
        static $xml;
     ...
     */
    static function getExtParamsXML($option)
    {
        static $xmls;
     ...
     */
    static function getExtTexts($option, $lang = '')
    {
    ...
    static function removeVariable($url, $var, $value = '')
    {
        if ($value == '') {
    ...
    static function getVariable($url, $var)
    {
        $value = null;
        $matches = array();
    ...
    static function extractVariable(&$url, $var)
    {
    ...
    /**
     * Fixes numerical variable by converting it to integer in the URI
     */
    static function fixVariable(&$uri, $varName)
    {
     ...
     */
    static function RemoveVariables(&$uri, $vars)
    {
        $query = array();
    ...
    static function ReplaceAll($search, $replace, $subject)
    {
     ...
     * @return boolean
     */
    static function UseAlias(&$params, $paramName)
    {
        $sefConfig = SEFConfig::getConfig();
	 ...
	 * @return string
	 */
	static function cleanDesc($text) {
		// Remove javascript
	 ...
	 * @return string
	 */
	static function clipDesc($text, $limit) {
	 ...
	 * @return string
	 */
	static function generateKeywords($desc, $blacklist, $count, $minLength) {
		// Remove any email addresses
		$regex = '/(([_A-Za-z0-9-]+)(\\.[_A-Za-z0-9-]+)*@([A-Za-z0-9-]+)(\\.[A-Za-z0-9-]+)*)/ix';
    ...
    static function GetSEFGlobalMeta() {
        return 'c26ad574e53311ba6045b9438dc61926'; // sef.global.meta
    }
    
    /**
     * Sends the request and follows redirects
     * 
     * @param string $url
     ...
     * @return object
     */
    static function PostRequestFollowRedirects($url, $referer = null, $_data = null, $method = 'post', $userAgent = null, $headers = null, $maxRedirs = 5)
    {
        $response = false;
        
        for ($redirs = 0; $redirs <= $maxRedirs; $redirs++) {
            $response = self::PostRequest($url, $referer, $_data, $method, $userAgent, $headers);
            
            // Check if we should redirect
            if (is_object($response) && $response->code >= 300 && $response->code <= 399) {
                // We got a redirect response code, try to parse new location from headers
                $matches = array();
                if (preg_match('/^Location:\s*(.*)$/im', $response->header, $matches)) {
                    // Found, update URL and send request again
                    $url = $matches[1];
                    continue;
                }
            }
    ...
    static function getNonSefVars(&$uri, $nonSefVars, $ignoreVars)
    {
    ...
    static function getHomeQueries($includeLang = true)
    {
	...
	static function getMenuItemSubDomains($id)
    {
    ...
    static function getAllSubdomains()
    {
	...
	static function getSubDomain($Itemid,$uri,&$titlepage)
    {
		$sefConfig=SEFConfig::getConfig();
		$titlepage=false;
     ...
     * @return bool
     */
    static function isItemidIgnored($option, $Itemid)
    {
        $params =& SEFTools::getExtParams($option);
    ...
    static function getDefaultParams($element)
    {
    ...
    static function getDefaultFilters($element)
    {
    ...
    /**
     * Redirects current page to given URL, handles different Joomla versions.
     * By default uses 303 code, if $moved is true, uses 301 code.
     * Optionally you can also specify message.
     * 
     * @param string $url
     * @param bool $moved
     ...
     * @param string $type
     */
    public static function redirect($url, $moved = false, $message = '', $type = 'message')
    {
        ...
        // Change http protocol to https if set to
        $sefConfig = SEFConfig::getConfig();
        if ($sefConfig->forceSsl) {
        ...
        // Handle redirect according to Joomla version
        if (version_compare(JVERSION, '3.2', 'ge')) {
            // New handling
            if ($message) {
                $app->enqueueMessage($message, $type);
            }
            $app->redirect($url, $moved);
        }
        else {
            // Old handling
            $app->redirect($url, $message, $type, $moved);
        }
