...
class SEFModelConfig extends SEFModel
{
    ...
    function getLists()
    {
        ...
        $lists['forceSsl']          = $this->booleanRadio('forceSsl',           $std_opt, $sefConfig->forceSsl);
            ...
            if (!is_array($vendor_currency)) {
                ...
                try {
                    $query = "SELECT `virtuemart_currency_id` AS `id`, CONCAT_WS(' ', `currency_name`, `currency_symbol`) AS `title`"
                        ." FROM `#__virtuemart_currencies` WHERE `virtuemart_currency_id` IN (".$allCurrencies.") AND published = '1' ORDER BY `ordering`, `currency_name`";
                    $db->setQuery($query);
        ...
        // Options to handle trailing slash redirect
        $opts = array();
        $opts[] = JHTML::_('select.option', COM_SEF_REDIRECT_SLASH_NEVER,       JText::_('COM_SEF_REDIRECT_SLASH_NEVER'));
        $opts[] = JHTML::_('select.option', COM_SEF_REDIRECT_SLASH_ALWAYS,      JText::_('COM_SEF_REDIRECT_SLASH_ALWAYS'));
        $opts[] = JHTML::_('select.option', COM_SEF_REDIRECT_SLASH_EXISTING,    JText::_('COM_SEF_REDIRECT_SLASH_EXISTING'));
        $lists['redirectSlash'] = JHTML::_('select.genericlist', $opts, 'redirectSlash', 'class="inputbox" size="1"', 'value', 'text', (int)$sefConfig->redirectSlash);
        ...
        $lists['alwaysAddItemid']       = $this->booleanRadio('alwaysAddItemid',        $std_opt, $sefConfig->alwaysAddItemid);
        ...
        $lists['chunkedExport']         = $this->booleanRadio('chunkedExport',          $std_opt, $sefConfig->chunkedExport);
    ...
    function store()
    {
        ...
        if ($page404 = $db->loadObject()){
            ...
            $sql = 'INSERT INTO #__content (title, alias, introtext, `fulltext`, state, catid, created, created_by, created_by_alias, modified, modified_by, publish_up, publish_down, images, urls, attribs, version, ordering, metakey, metadesc, access, hits, language) '.
            'VALUES ("404", "404", '.$db->quote($introtext).', "", "1", "0", '.$db->quote(date("Y-m-d H:i:s")).', "42", "", "0000-00-00 00:00:00", "0", "2001-01-01 00:00:00", "0000-00-00 00:00:00", "", "", "menu_image=-1\nitem_title=0\npageclass_sfx=\nback_button=\nrating=0\nauthor=0\ncreatedate=0\nmodifydate=0\npdf=0\nprint=0\nemail=0", "1", "0", "", "", "1", "0", "*");';
        }
        ...
        if (isset($subdomains['title']) && is_array($subdomains['title'])) {
