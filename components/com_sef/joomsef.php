...
//if (!defined('_JEXEC') && !JoomSEF::CheckAccess()) die();
...
class JoomSEF
{
    ...
    /**
     * Calls require_once in an isolated function to prevent overwriting any
     * local variables
     * 
     * @param string $file
     */
    protected static function _isolatedRequireOnce($file)
    {
        require_once($file);
    }
    ...
    static function build(&$uri,$check=false)
    {
        static $extsCache;
                    ...
                    // own extension block
                        ...
                        if (!$sefUrl || $check) {
                            ...
                            if (!isset($sefUrl->host) || !strlen($sefUrl->host)) {
                                // Don't use domain in URL if no host set
                                $url = '';
                            } else {
                                $url = JFactory::getURI()->getScheme().'://'.$sefUrl->host.JURI::root(true);
                            }
        ...
        // #17411: Fixed removing URI base from SEF URL
        $base = rtrim(JUri::base(true), '/').'/';
        if ($base != '/' && strpos($uri->getPath(), $base) === 0) {
            // Keep the starting slash
            $newPath = substr($uri->getPath(), strlen($base) - 1);
            $uri->setPath($newPath);
        }
    ...
    static function _prepareUriForCreate(&$params, &$uri) {
        $sefConfig = SEFConfig::getConfig();
     ...
     */
    static function getLangCode($sef = '') {
        if (empty($sef)) {
    ...
    static function getLanguage($uri) {
        $sefConfig = SEFConfig::getConfig();
        ...
        if (strlen($lang)) {
            ...
            // #14613: Instantiate new JLanguage object if required, otherwise
            // language overrides are not loaded and global texts may not be translated
            if (JFactory::getLanguage()->getTag() != $lang_code) {
                // Instantiate new JLanguage object and set it to JFactory
                $jLang = new JLanguage($lang_code, JFactory::getLanguage()->getDebug());
                JFactory::$language = $jLang;
                
                // Make sure the main application object uses our new JLanguage object
                $app = JFactory::getApplication();
    ...
    static function parse(&$uri)
    {
        ...
        if ($sefConfig->redirectSlash) {
            if ($path != '/' && $path != $uri->base(true).'/' && substr($path, -1) == '/') {
                // Check if the SEF URL without trailing slash exists if set to
                if ($sefConfig->redirectSlash == COM_SEF_REDIRECT_SLASH_EXISTING) {
                    $db->setQuery("SELECT `id` FROM `#__sefurls` WHERE `sefurl` = ".$db->quote(rtrim($path, '/'))." AND `enabled` = 1");
                    $sefId = $db->loadResult();
                    if ($sefId) {
                        // URL without trailing slash exists, redirect
                        SEFTools::redirect($uri->base(true).'/'.rtrim($path, '/'), false);
                    }
                }
                else if ($sefConfig->redirectSlash == COM_SEF_REDIRECT_SLASH_ALWAYS) {
                    SEFTools::redirect($uri->base(true).'/'.rtrim($path, '/'), false);
                }
            }
        }
        ...
        if ($sefConfig->langEnable && ($sefConfig->langPlacementJoomla == _COM_SEF_LANG_PATH)
            && (!$sefConfig->alwaysUseLangJoomla || !$sefConfig->alwaysUseLangHomeJoomla) && empty($_POST))
        {
    ...
    /**
     * Checks if the current protocol is SSL and redirects to https if necessary
     */
    static function _checkForcedSsl()
    {
        $sefConfig = SEFConfig::getConfig();
        $uri = JUri::getInstance();
        
        if ($sefConfig->forceSsl && empty($_POST) && $uri->getScheme() == 'http') {
            // Redirect current URL from http to https
            $uri->setScheme('https');
            SEFTools::redirect($uri->toString(), true);
        }
    }
    
    static function _parseSefUrl(&$uri, &$oldUri)
    {
        $mainframe = JFactory::getApplication();
        ...
        else
        {
            ...
            if ($sefConfig->record404) {
                $query = "SELECT * FROM `#__sefurls` WHERE `sefurl` = ".$db->quote($route);
                $db->setQuery($query);
                $results = $db->loadObjectList();

                if ($results) {
                    // we have it, so update counter
                    $db->setQuery("UPDATE `#__sefurls` SET `cpt`=(`cpt`+1) WHERE `sefurl` = ".$db->quote($route));
                    $db->query();
                }
                else {
                    // get trace info
                    ...
                    . " VALUES ('1', ".$db->quote($route).", '', {$traceinfo}, CURDATE())";
                    $db->setQuery($query);
                    $db->query();
                }
            }
    ...
    static function _buildQueryStringValues(&$qs, $name, $val) {
        if (is_array($val)) {
            foreach ($val as $k => $v) {
    ...
    static function _removeRightSlash($url)
    {
        if (strpos($url, '?') === false) {
            // No question mark
            return rtrim($url, '/');
    ...
    static function _get404vars($route = '')
    {
        $mainframe = JFactory::getApplication();
     ...
     */
    static function ksort_deep(&$arr) {
        ksort($arr);
        foreach ($arr as &$a) {
    ...
    static function _parseAlias($route, $vars)
    {
        $db = JFactory::getDBO();
        $sefConfig = SEFConfig::getConfig();
    ...
    static function _sendHeader($header)
    {
        if (!headers_sent()) {
    ...
    static function _parseRawRoute(&$uri)
    {
        $sefConfig = SEFConfig::getConfig();
    ...
    static function _createUri($uri)
    {
        $path = JURI::root(true);
        $url = '/'.$uri->toString(array('path', 'query', 'fragment'));
        JoomSefUri::updateUri($uri, $url);
    ...
    static function _restoreLang($lang = '')
    {
        if ($lang != '') {
    ...
    static function _isHomePage(&$uri, $altered = false)
    {
        $home_items=SEFTools::getHomeQueries();
        $langs=JLanguageHelper::getLanguages('lang_code');
    ...
    static function _getMenuTitle($option, $lang, $id = null, $string = null)
    {
        return self::_getMenuTitleLang($option, $lang, $id, $string);
    }
    
    static function _getMenuTitleLang($option, &$lang, $id = null, $string = null)
    {
        ...
        $row = false;
        if (isset($string)) {
            // Search by specific link
            if (!isset($stringCache[$string])) {
                $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `link` = ".$db->quote($string)." AND `published` > 0";
                $db->setQuery($query);
        ...
        elseif (isset($id) && $id != 0) {
            // Search by Itemid (most used)
            if (!isset($idCache[$id])) {
                $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `id` = ".(int)$id." AND `published` > 0";
                $db->setQuery($query);
            ...
            if (!isset($optionCache[$option])) {
                $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `link` = ".$db->quote('index.php?option='.$option)." AND `published` > 0";
                $db->setQuery($query);
                $optionCache[$option] = $db->loadObject();
                if (!$optionCache[$option]) {
                    // Try to extend the search for any link to component
                    $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `link` LIKE '".$db->escape('index.php?option='.$option)."%' AND `published` > 0";
                    $db->setQuery($query);
                    $optionCache[$option] = $db->loadObject();
                    if (!$optionCache[$option])
    ...
    static function _getMenuItemInfo($option, $task, $id = null, $string = null)
    {
        static $stringCache = array();
        static $idCache = array();
        static $optionCache = array();
        ...
        if (isset($id) && $id != 0) {
            if (!isset($idCache[$id])) {
                $query = "SELECT `$column` AS `name`, `params`$jfTranslate FROM `#__menu` WHERE `id` = ".(int)$id." AND `published` > 0";
                $db->setQuery($query);
                $idCache[$id] = $db->loadObject();
                if (!$idCache[$id])
                    $idCache[$id] = false;
        ...
        elseif (isset($string)) {
            if (!isset($stringCache[$string])) {
                $query = "SELECT `$column`AS `name`, `params` $jfTranslate FROM `#__menu` WHERE `link` = ".$db->quote($string)." AND `published` > 0";
                $db->setQuery($query);
                $stringCache[$string] = $db->loadObject();
                if (!$stringCache[$string])
                    $stringCache[$string] = false;
            ...
            if (!isset($optionCache[$option])) {
                ...
                $query = "SELECT `$column` AS `name`, `params` $jfTranslate FROM `#__menu` WHERE `link` = ".$db->quote('index.php?option='.$option)." AND `published` > 0";
                $db->setQuery($query);
                $optionCache[$option] = $db->loadObject();
                if (!$optionCache[$option]) {
                    // Try to extend the search for any link to component
                    $query = "SELECT `$column` AS `name`, `params` $jfTranslate FROM `#__menu` WHERE `link` LIKE '".$db->escape('index.php?option='.$option)."%' AND `published` > 0";
                    $db->setQuery($query);
                    $optionCache[$option] = $db->loadObject();
                    if (!$optionCache[$option])
                        $optionCache[$option] = false;
    ...
    static function _getCustomMenuTitle($option)
    {
        $db = JFactory::getDBO();
        $sefConfig = SEFConfig::getConfig();
     ...
     * @return string
     */
    static function _titleToLocation(&$title)
    {
        $sefConfig = SEFConfig::getConfig();
     ...
     */
    static function _toLowerCase($str)
    {
        $sefConfig = SEFConfig::getConfig();
    ...
    static function _utf8LowerCase($str)
    {
        if( function_exists('mb_convert_case') ) {
            $str = mb_convert_case($str, MB_CASE_LOWER, 'UTF-8');
        }
     ...
     * @return string
     */
    static function _sefGetLocation(&$uri, &$title, $task = null, $limit = null, $limitstart = null, $lang = null, $nonSefVars = null, $ignoreSefVars = null, $metadata = null, $priority = null, $pageHandled = false,$host=null, $sitemapParams = null)
    {
     ...
     * @return string
     */
    static function _storeLocation(&$data, $check = false, $removeItemid = false)
    {
        $mainframe = JFactory::getApplication();
        ...
        if (!$ignoreSource && isset($Itemid)) {
            $where .= " AND (`Itemid` = ".(int)$Itemid." OR `Itemid` IS NULL)";
        }
        ...
        if (!$sefConfig->useCache || !$realloc) {
            $query = "SELECT * FROM `#__sefurls` WHERE `origurl` = ".$db->quote(html_entity_decode(urldecode($url))).$where.' LIMIT 2';
            $db->setQuery($query);
            ...
            if (!$ignoreSource && isset($Itemid)) {
                if (isset($sefurls[$Itemid])) {
                    ...
                    $query = "UPDATE `#__sefurls` SET `Itemid` = ".(int)$Itemid." WHERE `id` = ".(int)$realloc->id." LIMIT 1";
                    $db->setQuery($query);
                    $db->query();
                }
                else {
        ...
        // URL not found, so lets create it
            ...
            $sql = "SELECT `id`, `origurl`, `Itemid`, `sefurl` FROM `#__sefurls` WHERE `sefurl` = ".$db->quote($temploc)." AND `origurl` != ''";
            $db->setQuery($sql);
            $row = $db->loadObject();
    ...
    static function enabled(&$plugin)
    {
        $mainframe = JFactory::getApplication();
     ...
     *
     */
    static function _checkRow(&$row, $ignoreSource, $Itemid, $url, &$metadata, $temploc, $priority, $option,$check=false,$host, $sitemapParams = null)
    {
        $realloc = null;
            ...
            if ($ignoreSource || (!$ignoreSource && (empty($Itemid) || $row->Itemid == $Itemid))) {
                ...
                if ($row->origurl == $url) {
                    ...
                    if (SEFTools::removeVariable($row->origurl, 'lang') == SEFTools::removeVariable($url, 'lang')) {
                        $db->setQuery("UPDATE `#__sefurls` SET `origurl` = ".$db->quote(SEFTools::removeVariable($row->origurl, 'lang'))." WHERE `id` = ".(int)$row->id." LIMIT 1");
        ...
        // URL not found
            ...
            if (!is_null($id)) {
                ...
                $query = "UPDATE `#__sefurls` SET `origurl` = ".$db->quote(html_entity_decode(urldecode($url)))."$sqlId, `priority` = ".(int)$priority." WHERE `id` = ".(int)$id." LIMIT 1";
                $db->setQuery($query);
     ...
     *
     */
    static function _saveNewURL($Itemid, &$metadata, $priority, $temploc, $url,$host, $sitemapParams)
    {
        $db = JFactory::getDBO();
        ...
        "VALUES (".$db->quote($temploc).", ".$db->quote(html_entity_decode(urldecode($url))).", ".(int)$priority . $val . $metavals . ", " . $traceinfo . ", '{$sm_indexed}', '{$sm_date}', '{$sm_frequency}', '{$sm_priority}',".$autolock.",'".$host."')";
        $db->setQuery($query);
    ...
    static function _removeURL($option,$item) {
        $db=JFactory::getDBO();
        $sefConfig = SEFConfig::getConfig();
        $cache=SEFCache::getInstance();
    ...
    static function getNonSEFURL($sefurl) {
        $db=JFactory::getDBO();
    ...
    static function _uriToUrl($uri, $removeVariables = null)
    {
     ...
     * @return int
     */
    static function _getPriorityDefault(&$uri)
    {
        $itemid = $uri->getVar('Itemid');
    ...
    static function _getDebugInfo($traceLevel = 3, $onlyUserInfo = false)
    {
        $debuginfo = '';
        $tr = 0;
        ...
        foreach ($trace as $row) {
            if (@$row['class'] == 'JRouterJoomsef' && @$row['function'] == 'buildSef') {
    ...
    static function OnlyPaidVersion()
    {
