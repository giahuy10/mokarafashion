...
class SefExt_com_tags extends SefExt
{
    ...
    {
        // Fix IDs - we must also handle arrays!
        $id = $uri->getVar('id');
        if (is_array($id)) {
    ...
    public function create(&$uri)
    {
        ...
        if (isset($vars['view'])) {
            switch ($vars['view']) {
                case 'tag':
                    if (isset($vars['id'])) {
                        // Add all the tags to URL
                        $ids = is_array($vars['id']) ? $vars['id'] : array($vars['id']);
                        foreach ($ids as $id) {
                            $tag = $this->getTag($id);
                            if (is_null($tag)) {
                                JoomSefLogger::Log('Tag with ID '.$id.' not set.', $this, 'com_tags');
                                return $uri;
                            }
        ...
        // Handle feeds
        if ($uri->getVar('format') == 'feed' && $uri->getVar('type')) {
            $title[] = $uri->getVar('type');
        }
