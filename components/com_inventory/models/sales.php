...
class InventoryModelSales extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$filter_Qcreated_from = (!empty($filter_created_from)) ? $this->isValidDate($filter_created_from) : null;

		if ($filter_Qcreated_from != null)
		{
			$query->where("a.created >= '" . $db->escape($filter_Qcreated_from) . "'");
		}

		$filter_created_to = $this->state->get("filter.created_to_dateformat");
		$filter_Qcreated_to = (!empty($filter_created_to)) ? $this->isValidDate($filter_created_to) : null ;

		if ($filter_Qcreated_to != null)
		{
			$query->where("a.created <= '" . $db->escape($filter_Qcreated_to) . "'");
		}
		...
		foreach ($items as $item)
		{
			...
			if (isset($item->user_id))
			{
				...
				foreach ($values as $value)
				{
					...
					{
						$db = JFactory::getDbo();
						$query = "SELECT id, CONCAT(name,'-',email) as value FROM #__users HAVING id LIKE '" . $value . "'";
						$db->setQuery($query);
