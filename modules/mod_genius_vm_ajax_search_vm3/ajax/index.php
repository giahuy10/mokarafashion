...
$absolute_path = str_replace('/modules/mod_genius_vm_ajax_search_vm3/ajax', '', $absolute_path);
...
require_once ( 'genius-functions.php' );
...
function str_highlight($text, $needle, $options = null, $highlight = null) {
    ...
    if ($highlight === null) {
        $highlight = '<span class="geniusHighlight" style="background-color: '.$highlightcolor.';">\1</span>';
    }
                  ...
                  if(count($ps_cat) > 0 && $showSKUint == 1) {
                      ...
                      $IntCounterCats = $IntCounterCats + 1;
                          $href_cat = "";
                            ...
                            $href_cat = str_replace('modules/mod_genius_vm_ajax_search_vm3/ajax/'.$seftoinsert, '', $href_cat);
...
$jArtBaseURL = str_replace('modules/mod_genius_vm_ajax_search_vm3/ajax/', '', $jArtBaseURL);
...
$strK2SEFURL = str_replace('modules/mod_genius_vm_ajax_search_vm3/ajax/', '', $strK2SEFURL);
...
if(count($ajx_single_product_parent->categories) > 1) {
 ...
 $prCatIDforURL = $ajx_single_product_parent->virtuemart_category_id;     
        ...
        $prCatIDforURL = $ajx_single_product->virtuemart_category_id;
		  	...
		  	$href = JRoute::_ ($href);
			  $href = str_replace('modules/mod_genius_vm_ajax_search_vm3/ajax/'.$seftoinsert, '', $href);
		  ...
		  if (empty($ajx_single_product->images[0]->file_url_thumb)) {
...
$templateCSScontent = file_get_contents('templates/'.$templateofchoice.'.css');
