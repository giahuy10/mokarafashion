			...
			else if (!empty($this->config->attrMandatoryValue[$descriptor]['empty']) && empty($attribute->value))
			{
				switch ($this->config->attrMandatoryValue[$descriptor]['empty']['action'])
				{
					case 'allow':
						break;
					case 'replace':
						$tag->setAttribute($attribute->name, $this->config->attrMandatoryValue[$descriptor]['empty']['replace_with']);
						break;
					case 'remove_attr':
						$tag->removeAttribute($attribute->name);
						break;
					case 'remove_tag':
						...
						break;
					default:
						throw new Exception('Internal error: invalid AttrMandatory rule action ' . $this->config->attrMandatoryValue[$descriptor]['empty']['action']);
						break;
				}
			}
