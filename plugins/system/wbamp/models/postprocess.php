...
class WbampModel_Postprocess
{
	...
	private $_sanitizers = array(
		'AMP_Img_Sanitizer' => array('content_max_width' => WbampHelper_Runtime::CONTENT_MAX_WIDTH),
		'AMP_Video_Sanitizer' => array('content_max_width' => WbampHelper_Runtime::CONTENT_MAX_WIDTH),
		'AMP_Audio_Sanitizer' => array('content_max_width' => WbampHelper_Runtime::CONTENT_MAX_WIDTH),
		'AMP_Iframe_Sanitizer' => array(
			'add_placeholder' => true,
			'content_max_width' => WbampHelper_Runtime::CONTENT_MAX_WIDTH
		)
	);
	...
	public function __construct($isAmpPage, $args = array())
	{
		...
		$this->args = $args;
		$this->config = wbArrayGet(
			$args,
			'config',
			new WbampModel_Config()
		);
	}
	...
	public function convert($rawContent)
	{
		...
		$wlCleaner = new WbampModelProcessor_Csscleanup($this->config, WbampHelper_Runtime::$params);
		...
		$wlCleaner = new WbampModelProcessor_Whitelist($this->config);
	...
	{
		$content = $rawContent;
		$regExpsRaw = WbampHelper_Runtime::$params->get('cleanup_regexp', $this->config->defaulCleanupRegexp);
		if (empty($regExpsRaw))
		{
			return $content;
		}
