...
class plgSystemWbampInstallerScript
{
	...
	private $_removableWbampPlugins = array(
		'wbamp' => array('k2'),
	...
	private function installLibraries($parent)
	{
		...
		foreach ($this->_removableWbampPlugins as $group => $pluginNames)
		{
			foreach ($pluginNames as $pluginName)
			{
	...
	private function fixPluginsOrder()
	{
		...
		{
			JFactory::getApplication()->enqueueMessage('Error fixing plugin order in database: ' . $e->getMessage());
			return false;
		}
	...
	private function ensurePluginsOrder($plugin1, $plugin2)
	{
		...
		if ($signature != $order || $plugins[$plugin2]->ordering == $plugins[$plugin1]->ordering)
		{
			...
			{
				$plugins[$plugin1]->ordering = $plugins[$plugin2]->ordering + 1;
			}
	...
	private function _doUninstall($parent)
	{
		...
		foreach ($this->_removableWbampPlugins as $pluginNames)
		{
			foreach ($pluginNames as $pluginName)
			{
				$this->_uninstallPlugin('wbamp', $pluginName);
			}
		}
