...
class WbampHelper_Route
{
	...
	/**
	 * Finds out whether current request is for a single page (an article, an item)
	 * or something else (category, a task, anything else)
	 *
	 * @return bool
	 */
	public static function isSingular($request)
	{
		static $isSingular = null;

		if (is_null($isSingular))
		{
			// deafult to false, to have less chances of invalid strcutured data
			$isSingular = false;
			$eventArgs = array(
				&$isSingular,
				$request
