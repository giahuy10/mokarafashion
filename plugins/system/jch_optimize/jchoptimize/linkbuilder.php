...
class JchOptimizeLinkBuilder extends JchOptimizelinkBuilderBase
{
	 ...
	 *
        ...
        protected function loadCssAsync($sUrl)
        {
                ...
                l.href = \'' . html_entity_decode($sUrl) . '\';
                var h = document.getElementsByTagName(\'head\')[0]; h.appendChild(l);
                };
                ';

                if ($this->params->get('pro_optimizeCssDelivery_loadFile', '1'))
                {
                        $sScript .= 'var raf = requestAnimationFrame || mozRequestAnimationFrame ||
                    webkitRequestAnimationFrame || msRequestAnimationFrame;
                if (raf) raf(cb);
                else ';
                }

                $sScript .= 'window.addEventListener(\'load\', cb);
/*]]>*/</script>
<noscript><link type="text/css" rel="stylesheet" property="stylesheet" href="' . $sUrl . '" /></noscript>'
                ...
                $this->oParser->setBodyHtml($sBodyHtml);
        }
        ...
        protected function getAsyncAttribute($iIndex)
        {
			...
			$sAsyncAttribute = $this->isXhtml() ? ' ' . $attr . '="' . $attr . '" ' : ' ' . $attr . ' ';
                ...
                {
                        return parent::getAsyncAttribute($iIndex);
                }
        }
